...
let [list_color,color_product,list_size,size_product] = ['',JSON.parse(document.getElementById('color_product').value),'',JSON.parse(document.getElementById('size_product').value)];
{
    "use strict";
    tinymce.init({
...
function submit_shared(ev){
    ...
    for (var i = 0; i < document.querySelectorAll('[id^="demo"]').length; i++) {
        for (var j = 0; j < document.querySelectorAll('[id^="demo"]')[i].querySelectorAll('.price_color').length; j++) {
            data.append(`price_color${i+1}[]`, document.querySelectorAll('[id^="demo"]')[i].querySelectorAll('.price_color')[j].value);
            data.append(`promotion_color${i+1}[]`, document.querySelectorAll('[id^="demo"]')[i].querySelectorAll('.promotion_color')[j].value);
            data.append(`quantity${i+1}[]`, document.querySelectorAll('[id^="demo"]')[i].querySelectorAll('.quantity')[j].value);
            data.append(`promotion_check${i+1}[]`, document.querySelectorAll('[id^="demo"]')[i].querySelectorAll('.promotion_check')[j].checked);
            if(document.querySelectorAll('[id^="demo"]')[i].querySelectorAll('.promotion_check')[j].checked){
                data.append('price[]', document.querySelectorAll('[id^="demo"]')[i].querySelectorAll('.promotion_color')[j].value);
            }else{
                data.append('price[]', document.querySelectorAll('[id^="demo"]')[i].querySelectorAll('.price_color')[j].value);
            }
        }
        for (var j = 0; j < document.querySelectorAll('[id^="demo"]')[i].querySelectorAll('.add-all-size [class^="size"]').length; j++) {
            data.append(`size${i+1}[]`, document.querySelectorAll('[id^="demo"]')[i].querySelectorAll('.add-all-size [class^="size"]')[j].className.replace('size',''));
        }
...
for(i = 0 ; i < size_product.length ; i++){
    list_size += `<div class="col-xs-3">
        <label class="checkbox-inline">
            <input type="checkbox" name="" onchange="check_size(this)" class="size_check" value="${size_product[i].vi}" /><span>${size_product[i].vi}</span>
        </label>
    </div>`;
}
// thêm nhiều field
        ...
        html += `
            <div style="margin-bottom:5px;">
                <div class="btn btn-primary col-ms-12 color_product" style="padding:0px; padding-top:5px; width:100%;text-align:left">
                    ...
                    <i style="float: right;padding-right:5px;" class="fa-2x fa fa-close remove" onclick="remove_color(${i+1})"></i>
                </div>
                <div id="demo${i+1}" class="collapse in form-group">
                    <div class="col-xs-12" style="padding:0px;">
                        ...
                        <div class="col-xs-12">
                            <label class="control-label">Giá chung cho màu sản phẩm</label>
                            <input type="text" name="" onblur="change_price_default(this)" value=""  placeholder="" class="form-control price_default">
                        </div>
                        <div class="col-xs-12" style="padding:0px;">
                            <label class="col-xs-12">Chọn size cho sản phẩm</label>
                            ${list_size}
                        </div>
                        <div class="col-xs-12 add-all-size" style="padding:5px;">
    ...
    html += `
        <div style="margin-bottom:5px;">
            <div class="btn btn-primary col-ms-12 color_product" style="padding:0px; padding-top:5px; width:100%;text-align:left">
                ...
                <i style="float: right;padding-right:5px;" class="fa-2x fa fa-close remove" onclick="remove_color(${number+1})"></i>
            </div>
...
function check_size(ev){
    if(ev.checked){
        size = `<div class="size${ev.value}">
            <div class="col-xs-12">
                <label class="col-xs-12" style="padding:0px; color:green;margin-top:5px; border-top:1px solid green;">Size ${ev.value}</label>
            </div>
            <div class="col-sm-6 col-xs-12">
                <label class="control-label" for="inputError">Giá của sản phẩm theo size </label>
                <input type="text" name="" value="${document.querySelector('.price_default').value}" placeholder="(Đơn vị VND)" onpaste ="return false" onkeypress=" return check_sale(event,this)" placeholder="" class="form-control price_color">
            </div>
            <div class="col-sm-6 col-xs-12">
                <label class="control-label" for="inputError">Giá khuyến mãi sản phẩm theo size</label>
                <input type="text" name="" value="" placeholder="(Đơn vị VND)" onpaste ="return false" onkeypress=" return check_sale(event,this)" class="form-control promotion_color">
            </div>
            <div class="col-sm-6 col-xs-12">
                <label class="control-label" for="inputError">Số lượng sản phẩm theo size</label>
                <input type="text" name="" value="" placeholder="" class="form-control quantity">
            </div>
            <div class="col-sm-6 col-xs-12 style_checkbox">
                <label class="checkbox-inline">
                    <input type="checkbox" name="" class="promotion_check" /><span>Hiển thị khuyến mãi theo size</span>
                </label>
            </div>
        </div>`;
        ev.closest(`[id^="demo"]`).querySelector('.add-all-size').insertAdjacentHTML('beforeend', size);
    }else{
        ev.closest(`[id^="demo"]`).querySelector('.add-all-size').removeChild(ev.closest(`[id^="demo"]`).querySelector(`.add-all-size .size${ev.value}`));
    }
}
function change_price_default(ev){
    for (i = 0; i < ev.closest('[id^="demo"]').querySelectorAll('.price_color').length; i++) {
        if(ev.closest('[id^="demo"]').querySelectorAll('.price_color')[i] != undefined && ev.closest('[id^="demo"]').querySelectorAll('.price_color')[i].value == ''){
            ev.closest('[id^="demo"]').querySelectorAll('.price_color')[i].value = ev.value;
        }
    }
}
...
function check_sale(evt,ev) {
   ...
   if (charCode > 31 && (charCode < 48 || charCode > 57) || charCode == 190 || charCode == 196){
      return false;
        ...
        if(Number(ev.value.slice(0,ev.selectionStart)+evt.key+ev.value.slice(ev.selectionStart,ev.value.length)) > Number(ev.closest('[id^="demo"]').querySelector('.price_color').value) && ev.className.indexOf('promotion_color') != '-1'){
